...
class Patient:
    ...
    def calculate_lab_test_charge(self):
        ...
        for i in self.__list_of_lab_test_ids:
            ...
            if(x == -1):
                self.__lab_test_charge = self.__lab_test_charge + 0
            
            
            else:
                lab_charge= LabTestRepository.get_test_charge(i)
                self.__lab_test_charge = self.__lab_test_charge + lab_charge
...
class LabTestRepository:
    ...
    def get_test_charge(lab_test_id):
        if lab_test_id not in LabTestRepository.__list_of_hospital_lab_test_ids:
            ...
            for i in LabTestRepository.__list_of_hospital_lab_test_ids:
                if(i == lab_test_id):
                    x = LabTestRepository.__list_of_hospital_lab_test_ids.index(i)
